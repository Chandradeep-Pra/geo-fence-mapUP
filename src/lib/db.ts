...
const databaseUrl = process.env.DATABASE_URL || "file:./data/geofence-alert.db";
const isFileDatabase = databaseUrl.startsWith("file:");
const databasePath = isFileDatabase
  ? resolve(process.cwd(), databaseUrl.replace("file:", "") || "./data/geofence-alert.db")
  : null;
...
async function getSqliteDb() {
  if (!databasePath) {
    throw new Error("SQLite database path is not available.");
  }
...
async function getPgPool() {
  if (!globalForDb.pgPool) {
    ...
    globalForDb.pgPool = new pgModule.Pool({
